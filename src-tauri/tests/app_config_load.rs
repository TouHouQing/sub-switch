...
use thq_switch_lib::{AppError, MultiAppConfig};

mod support;
use support::{ensure_test_home, reset_test_fs, test_mutex, TEST_APP_CONFIG_DIR};

fn cfg_path() -> PathBuf {
    let home = std::env::var("HOME").expect("HOME should be set by ensure_test_home");
    PathBuf::from(home).join(TEST_APP_CONFIG_DIR).join("config.json")
}
...
fn load_v1_config_returns_error_and_does_not_write() {
    ...
    let bak = home.join(TEST_APP_CONFIG_DIR).join("config.json.bak");
...
fn load_valid_v2_config_succeeds() {
    ...
    assert!(loaded
        .get_manager(&thq_switch_lib::AppType::Claude)
        .is_some());
    assert!(loaded.get_manager(&thq_switch_lib::AppType::Codex).is_some());
}
