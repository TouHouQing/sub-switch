...
#[path = "support.rs"]
mod support;
use support::{app_config_dir, create_test_state, ensure_test_home, reset_test_fs, test_mutex};
...
fn import_from_apps_does_not_rewrite_selected_app_directory() {
    ...
    let ssot_skill_dir = app_config_dir(home).join("skills").join("codex-skill");
...
fn sync_to_app_removes_disabled_and_orphaned_ssot_symlinks() {
    ...
    let ssot_dir = app_config_dir(home).join("skills");
...
fn uninstall_skill_creates_backup_before_removing_ssot() {
    ...
    let ssot_skill_dir = app_config_dir(home).join("skills").join("backup-skill");
...
fn restore_skill_backup_restores_files_to_ssot_and_current_app() {
    ...
    let ssot_skill_dir = app_config_dir(home).join("skills").join("restore-skill");
