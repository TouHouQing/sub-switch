...
fn sync_codex_provider_writes_config_without_touching_auth() {
    ...
    let auth_path = thq_switch_lib::get_codex_auth_path();
    let config_path = thq_switch_lib::get_codex_config_path();
...
requires_openai_auth = true
"#;
    thq_switch_lib::write_codex_live_atomic(&legacy_auth, Some(legacy_config))
...
"#
    ...
    let toml_text =
        fs::read_to_string(thq_switch_lib::get_codex_config_path()).expect("read config.toml");
...
fn sync_enabled_to_codex_writes_enabled_servers() {
    ...
    let path = thq_switch_lib::get_codex_config_path();
    ...
    thq_switch_lib::sync_enabled_to_codex(&config).expect("sync codex");
...
fn sync_enabled_to_codex_returns_error_on_invalid_toml() {
    ...
    let err = thq_switch_lib::sync_enabled_to_codex(&config).expect_err("sync should fail");
    match err {
        thq_switch_lib::AppError::Toml { path, .. } => {
            assert!(
                path.ends_with("config.toml"),
                "path should reference config.toml"
            );
        }
        thq_switch_lib::AppError::McpValidation(msg) => {
            assert!(
...
fn sync_codex_provider_missing_auth_returns_error() {
    ...
    match err {
        thq_switch_lib::AppError::Config(msg) => {
    ...
    // 确认未产生任何 live 配置文件
    assert!(
        !thq_switch_lib::get_codex_auth_path().exists(),
        "auth.json should not be created on failure"
    );
    assert!(
        !thq_switch_lib::get_codex_config_path().exists(),
...
"#;
    ...
    thq_switch_lib::write_codex_live_atomic(&auth, Some(config_text))
    ...
    let stored_auth: serde_json::Value =
        thq_switch_lib::read_json_file(&auth_path).expect("read auth");
...
"#;
    ...
    let err = thq_switch_lib::write_codex_live_atomic(&auth, Some(config_text))
        .expect_err("config write should fail when target is directory");
    match err {
        thq_switch_lib::AppError::Io { path, .. } => {
            assert!(
                path.ends_with("config.toml"),
                "io error path should point to config.toml"
            );
        }
        thq_switch_lib::AppError::IoContext { context, .. } => {
            assert!(
...
"#,
    ...
    let mut config = MultiAppConfig::default();
    let changed = thq_switch_lib::import_from_codex(&mut config).expect("import codex");
...
fn sync_claude_enabled_mcp_projects_to_user_config() {
    ...
    thq_switch_lib::sync_enabled_to_claude(&config).expect("sync Claude MCP");

    let claude_path = thq_switch_lib::get_claude_mcp_path();
...
fn import_from_claude_merges_into_config() {
    ...
    let changed = thq_switch_lib::import_from_claude(&mut config).expect("import from claude");
...
fn create_backup_skips_missing_file() {
    ...
    reset_test_fs();
    let home = ensure_test_home();
    let config_path = app_config_dir(home).join("config.json");
...
fn sync_gemini_packycode_sets_security_selected_type() {
    ...
    // security field is written to ~/.gemini/settings.json, not THQ Switch settings.json
...
fn export_sql_returns_error_for_invalid_path() {
    ...
    let invalid_parent = if cfg!(windows) {
        std::env::temp_dir().join("thq-switch-test-invalid<>dir")
    } else {
...
#[test]
fn import_sql_rejects_non_thq_switch_backup() {
    let _guard = test_mutex().lock().expect("acquire test mutex");
    reset_test_fs();
    let home = ensure_test_home();
    ...
    let import_path = home.join("not-thq-switch.sql");
    fs::write(&import_path, "CREATE TABLE x (id INTEGER);").expect("write import sql");

    let err = state
        .db
        .import_sql(&import_path)
        .expect_err("non-THQ Switch sql should be rejected");
...
#[test]
fn import_sql_accepts_thq_switch_exported_backup() {
    let _guard = test_mutex().lock().expect("acquire test mutex");
    reset_test_fs();
    let home = ensure_test_home();
    ...
    let state = create_test_state_with_config(&config).expect("create test state");
    let export_path = home.join("thq-switch-export.sql");
    state
        .db
        .export_sql(&export_path)
        .expect("export should succeed");
