...
fn restore_live_settings_for_provider_backfill(
    ...
    // `modelCatalog` is a THQ Switch-private field whose SSOT is the DB. Live's
...
pub fn read_live_settings(app_type: AppType) -> Result<Value, AppError> {
    match app_type {
        AppType::Codex => {
            ...
            // `modelCatalog` is a THQ Switch private field that lives only in
            // the DB SSOT plus the `thq-switch-model-catalog.json` projection
...
/// into the THQ Switch database. Each provider found will be added to the
