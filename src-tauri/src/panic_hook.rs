...
fn default_app_config_dir() -> PathBuf {
    dirs::home_dir()
        ...
        .join(crate::identity::APP_CONFIG_DIR_NAME)
}
...
"#
        ...
        if let Ok(mut file) = OpenOptions::new().create(true).append(true).open(&log_path) {
            ...
            // 记录日志文件位置到 stderr
            eprintln!("\n[THQ-Switch] Crash log saved to: {}", log_path.display());
        }
...
mod tests {
    ...
    fn test_crash_log_path() {
        ...
        assert!(path.to_string_lossy().contains(crate::identity::APP_CONFIG_DIR_NAME));
    }
