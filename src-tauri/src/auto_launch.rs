...
/// 将 `/path/to/THQ Switch.app/Contents/MacOS/thq-switch` 转换为 `/path/to/THQ Switch.app`
#[cfg(target_os = "macos")]
...
fn get_auto_launch() -> Result<AutoLaunch, AppError> {
    let app_name = crate::identity::APP_DISPLAY_NAME;
    let exe_path =
...
mod tests {
    ...
    fn test_get_macos_app_bundle_path_valid() {
        let exe_path = std::path::Path::new("/Applications/THQ Switch.app/Contents/MacOS/thq-switch");
        let result = get_macos_app_bundle_path(exe_path);
        assert_eq!(
            result,
            Some(std::path::PathBuf::from("/Applications/THQ Switch.app"))
        );
    }
    ...
    fn test_get_macos_app_bundle_path_with_spaces() {
        let exe_path =
            std::path::Path::new("/Users/test/My Apps/THQ Switch.app/Contents/MacOS/thq-switch");
        let result = get_macos_app_bundle_path(exe_path);
        assert_eq!(
            result,
            Some(std::path::PathBuf::from(
                "/Users/test/My Apps/THQ Switch.app"
            ))
        );
    }
    ...
    fn test_get_macos_app_bundle_path_not_in_bundle() {
        let exe_path = std::path::Path::new("/usr/local/bin/thq-switch");
        let result = get_macos_app_bundle_path(exe_path);
    ...
    fn test_get_macos_app_bundle_path_dev_build() {
        ...
        let exe_path = std::path::Path::new("/Users/dev/project/target/debug/thq-switch");
        let result = get_macos_app_bundle_path(exe_path);
        assert_eq!(result, None);
    }
}
