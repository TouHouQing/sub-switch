...
/**
 * Hermes Agent configuration API (THQ Switch side).
 *
 * THQ Switch intentionally keeps its Hermes surface minimal — deep configuration
 * (model, agent behavior, env vars, skills, cron, logs, analytics) lives in
 * the Hermes Web UI at http://127.0.0.1:9119. THQ Switch only reads the `model`
