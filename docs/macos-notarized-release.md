...
2. **Unsigned fallback**: no Apple account required. The workflow publishes a guided DMG, a ZIP archive, and an installer script. The DMG window tells users to drag the app into Applications, run the embedded `install.sh` helper from Terminal, and then open the app.
...
- `THQ-Switch-<tag>-macOS.dmg`: generated with `appdmg`, showing a 1 -> 2 -> 3 installation guide, the app, an Applications shortcut, and an embedded `install.sh` helper to run from Terminal.
