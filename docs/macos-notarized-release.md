...
2. **Unsigned fallback**: no Apple account required. The workflow publishes a guided DMG, a ZIP archive, and an installer script. The DMG window tells users to drag the app into Applications, open Terminal from the DMG, drag the embedded shell helper into Terminal, and press Return.
