...
This directory contains the Flatpak manifest (`com.ccswitch.desktop`) for THQ Switch, used to convert the generated `.deb` artifact into an installable `.flatpak` package via CI or local builds.
...
flatpak build-bundle --runtime-repo=https://flathub.org/repo/flathub.flatpakrepo flatpak-repo THQ-Switch-Linux.flatpak com.ccswitch.desktop
```

4) Install and run:

```bash
flatpak install --user ./THQ-Switch-Linux.flatpak
